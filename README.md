...
- **Leads** (required) — one row per lead/opportunity. Columns: `Company, Industry, Sub Industry, Studio Type, Website, City, Country, Email, Mobile App Status, Booking Platform Used, Business Pain, Recommended Solution, Campaign, Date Added, Email Sent Date, Follow Up Date, Follow Up 2 Date, Follow Up 3 Date, Follow Up 4 Date, Response Date, Meeting Date, Proposal Date, Won Date, Lost Date, Stage, Deal Value`.
  ...
  - `Email Sent Date` is the 1st outreach touch. `Follow Up Date` / `Follow Up 2 Date` / `Follow Up 3 Date` / `Follow Up 4 Date` are the 2nd through 5th outreach touches — all optional, leave blank if that follow-up hasn't happened yet. These power the **Outreach Persistence** card and the per-lead outreach dots in the Lead Directory table.
